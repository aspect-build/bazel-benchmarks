################################################################################
# 2. Linking node_modules with lockfile pre-generated
################################################################################
...
if true; then
  ...
  cd npm_install
  echo "setup..."
  bazel clean --expunge && npm cache clean --force && rm -rf node_modules
...
if true; then
  ...
  cd yarn_install
  echo "setup..."
  yarn cache clean --all && rm -f snapshots.js && rm -rf node_modules
...
if true; then
  ...
  cd yarn_install
  echo "setup..."
  bazel clean --expunge && yarn cache clean --all && rm -rf node_modules
...
if true; then
  ...
  echo "setup..."
  rm -rf $(pnpm store path) && rm -rf node_modules
...
# ------------------------------------------------------------------------------
if true; then
  perf="2_npm_translate_lock"
  echo -e "\n\n\n================================================================================\n$perf"
  echo "================================================================================"
  cd npm_translate_lock
  echo "setup..."
  bazel clean --expunge && rm -rf $(bazel info repository_cache) && rm -rf node_modules
  bazel query @npm//...
...
# npm_translate_lock (cached)
# ✔ Populated external repositories & external repository cache
# ✔ Lock file
# ✗ node_modules (in bazel output tree)
# ------------------------------------------------------------------------------
if true; then
  perf="2_npm_translate_lock_cached"
  echo -e "\n\n\n================================================================================\n$perf"
  echo "================================================================================"
  cd npm_translate_lock
  echo "setup..."
